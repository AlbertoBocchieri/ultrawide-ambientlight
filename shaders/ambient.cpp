...
struct alignas(16) AmbientParams
{
    ...
    UINT mode = AMBIENT_CONVERT;
    UINT encoding = AMBIENT_SDR;
    UINT outputLinear = 0;
    float mip = 0;
};
...
HRESULT AmbientBackground::Render(ID3D11DeviceContext* context, TextureView desktop, TextureView output,
                                  RECT video, UINT encoding, float radius, float strength, int transitionMs)
{
    D3D11_TEXTURE2D_DESC outputDesc = {}, sourceDesc = {};
    output.GetTexture()->GetDesc(&outputDesc);
    desktop.GetTexture()->GetDesc(&sourceDesc);
    if (video.left < 0 || video.top < 0 || video.right > LONG(sourceDesc.Width) ||
        video.bottom > LONG(sourceDesc.Height) || video.right <= video.left || video.bottom <= video.top)
        return E_INVALIDARG;
    const UINT videoWidth = video.right - video.left, videoHeight = video.bottom - video.top;
    const float scale = std::min(1.0f, 320.0f / std::max(outputDesc.Width, outputDesc.Height));
    const UINT w = std::max(1u, UINT(std::lround(outputDesc.Width * scale)));
    const UINT h = std::max(1u, UINT(std::lround(outputDesc.Height * scale)));
    ...
    HRESULT hr = m_linear.RecreateTexture(m_device.Get(), fp16, videoWidth, videoHeight, 0, true);
    RETURN_IF_FAILED(hr);
    ...
    p.rect[0] = float(video.left);
    p.rect[1] = float(video.top);
    dispatch(m_linear, desktop);
    context->GenerateMips(m_linear.GetSRV());

    const float aspect = float(outputDesc.Width) / outputDesc.Height;
    const float sx = std::min(1.0f, aspect / (float(videoWidth) / videoHeight));
    const float sy = std::min(1.0f, (float(videoWidth) / videoHeight) / aspect);
    p.mode = AMBIENT_CROP;
    p.rect[0] = (1 - sx) / 2;
    p.rect[1] = (1 - sy) / 2;
    p.rect[2] = sx;
    p.rect[3] = sy;
    p.mip = std::max(0.0f, std::log2(std::max(videoWidth * sx / w, videoHeight * sy / h)));
    dispatch(m_levels[0], m_linear);

    const float r = radius * scale;
    int passes = r > 0
        ? std::clamp(int(std::ceil(std::log(1 + r*r / (1.8f*1.8f)) / std::log(4.0f))), 2, 9)
        : 0;
    float offset = passes ? r / std::sqrt(std::pow(4.0f, passes) - 1) : 0;
    if (offset < 1 && passes > 2)
        offset = r / std::sqrt(std::pow(4.0f, --passes) - 1);
    if (offset > 1.8f && passes < 10)
        offset = r / std::sqrt(std::pow(4.0f, ++passes) - 1);
    UINT passWidth = w, passHeight = h;
    for (int i = 0; i < passes; ++i) {
        if (passWidth == 1 && passHeight == 1)
        {
            passes = i;
            break;
        }
        p.mode = AMBIENT_DOWNSAMPLE;
        p.step[0] = offset / passWidth;
        p.step[1] = offset / passHeight;
        passWidth = std::max(1u, passWidth / 2);
        passHeight = std::max(1u, passHeight / 2);
        hr = m_levels[i+1].RecreateTexture(m_device.Get(), fp16, passWidth, passHeight);
        RETURN_IF_FAILED(hr);
    ...
    for (int i = passes - 1; i >= 0; --i) {
        ...
        p.mode = AMBIENT_UPSAMPLE;
        p.step[0] = offset / d.Width;
        p.step[1] = offset / d.Height;
        dispatch(m_levels[i], m_levels[i+1]);
    }
    LARGE_INTEGER now, frequency;
    QueryPerformanceCounter(&now);
    QueryPerformanceFrequency(&frequency);
    const double elapsed = double(now.QuadPart - m_lastTime) * 1000 / frequency.QuadPart;
    p.mode = AMBIENT_SMOOTH;
    ...
    p.mode = AMBIENT_COMPOSE;
    p.strength = strength;
    p.rect[0] = float(video.left);
    p.rect[1] = float(video.top);
    p.rect[2] = float(video.right);
    p.rect[3] = float(video.bottom);
